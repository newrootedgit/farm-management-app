...
export default function TransplantView({ tasks, onTaskClick, onViewLog, showCompleted, showOverdue = true }: TransplantViewProps) {
  const today = useMemo(() => {
    const d = new Date();
    d.setHours(0, 0, 0, 0);
    return d;
  ...
  const pendingCount = todayTransplant.filter((t) => !(t.status === 'COMPLETED' && t.completedBy)).length;
  // Count tasks that are fully complete (have logs)
  const completedCount = todayTransplant.filter((t) => t.status === 'COMPLETED' && t.completedBy).length;

  // Overdue transplant tasks
  const overdueTransplant = useMemo(() => {
    return transplantTasks
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Overdue Transplants */}
      {overdueTransplant.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold text-red-600 dark:text-red-400">⚠️ Overdue Transplants</h2>
            <span className="text-sm text-red-600 dark:text-red-400">
              {overdueTransplant.length} overdue
            </span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {overdueTransplant.map((task) => {
                ...
                <div
                  ...
                  className={`border rounded-lg p-4 bg-card transition-all cursor-pointer ${
                    taskIsOverdue
                      ...
                      : `${TRANSPLANT_STYLE.border} ${TRANSPLANT_STYLE.hoverBorder} hover:shadow-md`
                  }`}
                >
                  {/* Header */}
                  <div className="flex items-start justify-between mb-3">
                    ...
                    >
                      {taskIsOverdue ? '⚠️ OVERDUE' : isFullyComplete ? 'Completed' : 'Move to Light'}
                    </span>
                    <span className="text-xs text-muted-foreground">
                      {task.orderItem?.order?.orderNumber}
                    </span>
